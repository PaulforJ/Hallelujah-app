...
const sb = {
  ...
  async uploadMedia(userId, file) {
    ...
    // Try upload
    const { data: upData, error } = await supabase.storage.from("post-images").upload(path, file, {
      cacheControl: "3600",
      upsert: true,
    });
    if(error) {
...
function ExploreScreen() {
  ...
  if(viewProfile) return <UserProfileScreen userId={viewProfile.id} username={viewProfile.username} onBack={()=>setViewProfile(null)}/>;
...
function CreateScreen({user, onPosted}) {
  ...
  return (
    <div style={{padding:"16px 14px"}}>
      ...
      <textarea value={text} onChange={e=>setText(e.target.value)} placeholder="What's on your heart today? Share a scripture, testimony, or praise..." style={{width:"100%",minHeight:140,background:"transparent",border:"none",borderBottom:`1px solid ${C.border}`,borderRadius:0,padding:"14px 0",color:C.cream,fontSize:16,lineHeight:1.8,fontFamily:"'DM Sans',sans-serif",outline:"none",resize:"none",boxSizing:"border-box"}}/>
      {preview && (
...
function ProfileScreen({user, onSignOut}) {
  ...
  useEffect(()=>{
    Promise.all([sb.getMyPosts(user.id), sb.getProfile(user.id)])
      .then(([p, prof])=>{ setPosts(p); setProfile(prof); setBio(prof?.bio||""); })
  ...
  const saveProfile = async () => {
    ...
    if(avatarFile) { avatar_url = await sb.uploadAvatar(user.id, avatarFile); }
    await sb.updateProfile(user.id, { bio, avatar_url });
    setProfile(prev=>({...prev, bio, avatar_url}));
  ...
  return (
    <div>
      <div style={{height:140,background:`linear-gradient(160deg,${C.gold}33,${C.goldDim}22,${C.bg})`,position:"relative",marginBottom:56,borderBottom:`1px solid ${C.border}44`}}>
        <div style={{position:"absolute",bottom:-44,left:"50%",transform:"translateX(-50%)",cursor:"pointer"}} onClick={()=>editing&&avatarRef.current?.click()}>
      ...
      <div style={{textAlign:"center",padding:"0 20px",marginBottom:16}}>
        ...
        {editing
          ...
          : <div style={{color:C.muted,fontSize:13,fontFamily:"'DM Sans',sans-serif",marginTop:4,lineHeight:1.5}}>{profile?.bio||"✝️ A child of God"}</div>}
