...
// ─── POST CARD ───────────────────────────────────────────────
function PostCard({post, liked, onLike, user, onCommentAdded}) {
    ...
    <>
      <div onClick={handleDoubleTap} style={{background:C.card,borderRadius:16,marginBottom:2,overflow:"hidden",position:"relative",userSelect:"none"}}>
        <div style={{display:"flex",alignItems:"center",gap:10,padding:"14px 14px 10px"}}>
          ...
          <div style={{flex:1}}>
            <div style={{color:C.white,fontWeight:600,fontSize:14,fontFamily:"'DM Sans',sans-serif"}}>@{post.username}</div>
...
// ─── FEED SCREEN ─────────────────────────────────────────────
function FeedScreen({posts, likedIds, onLike, user, loading, onCommentAdded}) {
    ...
    <div>
      ...
      <div style={{padding:"12px 0"}}>
        {posts.length===0
          ...
          : posts.map(p=><PostCard key={p.id} post={p} liked={likedIds.includes(p.id)} onLike={()=>onLike(p)} user={user} onCommentAdded={onCommentAdded}/>)}
      </div>
    </div>
  );
}
...
function CreateScreen({user, onPosted}) {
  ...
  const post = async () => {
    ...
    setStage("uploading");
    let image_url = null;
    if(media){ try{ image_url = await sb.uploadMedia(user.id, media); }catch(e){} }
    setStage("posting");
    ...
    <div style={{padding:"16px 14px"}}>
      ...
      {stage!=="flagged" && (
        <div style={{display:"flex",gap:8,padding:"10px 12px",background:"#0A0F18",borderRadius:10,marginTop:14,border:`1px solid ${C.border}`}}>
          <CrossSvg size={14}/>
          <p style={{color:"#3A4E5A",fontSize:11,fontFamily:"'DM Sans',sans-serif",margin:0,lineHeight:1.5}}>Posts are reviewed by AI to keep this community Christ-centered.</p>
        </div>
      )}
      <button onClick={stage==="flagged"?()=>{setStage("idle");setFlagReason("");}:post} disabled={(!text.trim()&&stage!=="flagged")||busy} style={{width:"100%",padding:"15px",background:stage==="done"?"#0E2A1A":stage==="flagged"?C.card:`linear-gradient(135deg,${C.gold},${C.goldDim})`,color:stage==="done"?C.green:stage==="flagged"?C.muted:"#080C10",border:stage==="flagged"?`1px solid ${C.border}`:"none",borderRadius:16,fontSize:15,fontWeight:700,fontFamily:"'DM Sans',sans-serif",cursor:"pointer",marginTop:16,transition:"all 0.25s"}}>
...
function ProfileScreen({user, onSignOut}) {
  ...
  useEffect(()=>{ sb.getMyPosts(user.id).then(d=>setPosts(d)).finally(()=>setLoading(false)); },[]);
      ...
      <div style={{padding:"0 14px"}}>
        ...
        : posts.map(p=>(
          <div key={p.id} style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:14,padding:"12px 14px",marginBottom:10}}>
            ...
            {p.image_url && <img src={p.image_url} alt="" style={{width:"100%",maxHeight:180,objectFit:"cover",borderRadius:8,marginBottom:8}} onError={e=>e.target.style.display="none"}/>}
...
function BottomNav({tab, setTab, isAdmin}) {
  ...
  return (
    <div style={{position:"fixed",bottom:0,left:0,right:0,background:`${C.bg}F2`,backdropFilter:"blur(16px)",WebkitBackdropFilter:"blur(16px)",borderTop:`1px solid ${C.border}`,display:"flex",alignItems:"center",padding:"8px 0 20px",zIndex:50}}>
...
export default function App() {
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,color:C.cream,fontFamily:"'DM Sans',sans-serif"}}>
      <div style={{padding:"14px 18px 12px",display:"flex",alignItems:"center",justifyContent:"space-between",position:"sticky",top:0,background:`${C.bg}F5`,backdropFilter:"blur(12px)",WebkitBackdropFilter:"blur(12px)",zIndex:40,borderBottom:`1px solid ${C.border}33`}}>
        <div style={{display:"flex",alignItems:"center",gap:7}}>
          <CrossSvg size={20}/>
          <span style={{fontSize:20,fontWeight:700,color:C.gold,fontFamily:"'Playfair Display',serif"}}>Hallelujah</span>
        </div>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          {isAdmin && <span style={{background:`${C.gold}22`,color:C.gold,fontSize:10,fontWeight:700,padding:"3px 8px",borderRadius:8,fontFamily:"'DM Sans',sans-serif",letterSpacing:"0.06em"}}>ADMIN</span>}
          <div style={{width:7,height:7,borderRadius:"50%",background:C.green,boxShadow:`0 0 8px ${C.green}`}}/>
        </div>
      </div>
      <div style={{paddingBottom:100}}>
        {tab==="home"          && <FeedScreen posts={posts} likedIds={likedIds} onLike={handleLike} user={auth.user} loading={feedLoading} onCommentAdded={handleCommentAdded}/>}
