  ...
  const submit = async () => {
    ...
    try {
      if(mode==="signup") {
        ...
        const s = await sb.signIn(email, password); if(!s.error) { onAuth(s.user, s.access_token); } else { setError("✅ Account created! Please sign in."); setMode("login"); }
      } else {
        const d = await sb.signIn(email, password);
        if(d.error){setError(d.error.message);return;}
        if(d.user && d.access_token){ onAuth(d.user, d.access_token); } else { setError(d.error?.message || "Login failed. Please try again."); }
      }
