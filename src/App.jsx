...
// ─── POST CARD ───────────────────────────────────────────────
function PostCard({post, liked, onLike, user, onCommentAdded, authorAvatar}) {
    ...
    <div style={{display:"flex",gap:16,overflowX:"auto",padding:"14px 16px 12px",scrollbarWidth:"none",msOverflowStyle:"none",borderBottom:`1px solid ${C.border}55`}}>
      ...
      <div style={{flexShrink:0,display:"flex",flexDirection:"column",alignItems:"center",gap:5}}>
        <div style={{width:56,height:56,borderRadius:"50%",background:`linear-gradient(135deg,${C.gold},${C.goldDim})`,display:"flex",alignItems:"center",justifyContent:"center",border:`2px dashed ${C.gold}88`,position:"relative"}}>
          {avatarMap?.[userId] ? <img src={avatarMap[userId]} alt="" style={{width:56,height:56,borderRadius:"50%",objectFit:"cover"}}/> : <Avatar name={username} size={52}/>}
    ...
    <div>
      ...
      <div style={{padding:"4px 0"}}>
        {posts.length===0
          ...
          : posts.map(p=><PostCard key={p.id} post={p} liked={likedIds.includes(p.id)} onLike={()=>onLike(p)} user={user} onCommentAdded={onCommentAdded} authorAvatar={avatarMap?.[p.user_id]}/>)}
      </div>
    </div>
  );
}
