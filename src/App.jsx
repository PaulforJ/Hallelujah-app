  ...
  const post = async () => {
    ...
    if(media) {
      try {
        ...
        if(!image_url) { setFlagReason("Upload failed — check storage bucket."); setStage("flagged"); return; }
      } catch(e) {
        setFlagReason("Upload error: " + (e?.message || "Unknown"));
        setStage("flagged");
        return;
      }
    }
...
function ProfileScreen({user, onSignOut}) {
  ...
  const saveProfile = async () => {
    ...
    try {
      ...
      if(avatarFile) {
        const uploaded = await sb.uploadAvatar(user.id, avatarFile);
        if(uploaded) avatar_url = uploaded;
      }
      const { data, error } = await sb.updateProfile(user.id, { bio: bio.trim(), avatar_url });
      if(error) { alert("Save failed: " + error.message); }
      else {
        setProfile(prev=>({...prev, bio: bio.trim(), avatar_url}));
